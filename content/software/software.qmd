...
[JUDI](https://github.com/slimgroup/JUDI.jl) and its derived packages are a Julia framework for PDE constraints optimization that provides a high-level linear algebra user interface, while delivering state-of-the-art computational performance through the use of [Devito](https://github.com/devitocodes/devito).
...
From this base package, several extensions have been developed to enhance the capabilities and applications.


**[JUDI4Cloud](https://github.com/slimgroup/JUDI4Cloud.jl)**

JUDI4Cloud is an extension of [JUDI] that runs the core computation, which is the task-parallel wave equation solver, on Azure using Azure Batch. This software interfaces with [AzureClusterlessHPC.jl] for the Julia interface with Azure Batch.

**[TimeProbeSeismic](https://github.com/slimgroup/TimeProbeSeismic.jl)**

Wave-equation-based inversion with random trace estimation-based gradient computation. This method drastically reduces the memory imprint of the adjoint state while managing the loss of accuracy by carefully choosing the probing vector within the range of the wavefield.
...
Photocacoustic imaging and inversion software. This software is built on top of [JUDI.jl](https://github.com/slimgroup/JUDI.jl) and [Devito](https://github.com/devitocodes/devito) to offer a high-level interface and state-of-the-art computational performance. It is also designed to be used in combination with a machine learning framework, with differentiation rules defined to use the photacoustic propagators as layers/summaries.

**[pysegy][https://github.com/mloubout/pysegy]**

A Python implementation of [SegyIO.jl](https://github.com/slimgroup/SegyIO.jl) that provides a high-level interface to read, write, and scan SEGY files. This software is intended to enable the handling of large-scale datasets through lazy data loading and header-based indexing. It is fully compatible with any File System compatible with the `fsspec` Python package, such as Azure Blob Storage, S3, and others. It is also compatible with the [Dask](https://dask.org/) framework, enabling parallel processing of large datasets.

## Others

[XConv](https://github.com/slimgroup/XConv.jl):

This repository implements a high-performance, low-memory convolutional layer in both Julia (for Flux.jl) and Python (for PyTorch). The convolutional layer has virtually zero memory imprint for training using randomized linear algebra to compute an unbiased estimate of the gradient with respect to the weights. Additionally, a byte-only implementation of the ReLU layer results in a memory reduction by a factor of 2 for full networks.

[SlimOptim](https://github.com/slimgroup/SlimOptim.jl):
This software is a translation of three of our main optimization algorithms in Julia: SPG, PQN, and Linearized Bregman. The implementation is not aimed at computational optimality for minor problems, but rather for use in the context of PDE-constrained optimization, where the cost of solving the PDE is the primary computational expense. The SPG and PQN implementations are inspired by and adapted from [minConf](https://www.cs.ubc.ca/~schmidtm/Software/minConf.html).


# Contributor

[InvertibleNetworks](https://github.com/slimgroup/InvertibleNetworks.jl):
Julia's implementation of invertible networks. Invertible networks are fully reversible and do not need to store intermediate state variables for training, making them virtually memory-free.

[SLIM softwares](https://github.com/slimgroup):
I am the primary maintainer and manager of our software stack (CI, versioning, Julia registration,...).
